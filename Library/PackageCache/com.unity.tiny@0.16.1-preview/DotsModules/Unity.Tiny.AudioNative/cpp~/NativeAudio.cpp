...
ma_device* maDevice;          // must be 8-byte aligned
struct UserData
{
    void* dummy;
};
...
ZEROPLAYER_EXPORT
void ZEROPLAYER_CALL freeAudio(uint32_t clipID) 
{
...
ZEROPLAYER_EXPORT
uint32_t ZEROPLAYER_CALL startLoad(const char* fname)
{
    if (!audioInitialized) return 0;

    ++clipIDPool;

    if (strstr(fname, "!audiotest!")) {
        size_t size = 0;
        void* mem = createTestWAV(fname, &size);
        clipMap[clipIDPool] = new SoundClip(mem, size);
    }
    else {
        clipMap[clipIDPool] = new SoundClip(std::string(fname));
    }

    LOGE("startLoad(%s) id=%d", fname, clipIDPool);
    return clipIDPool;
}
...
ZEROPLAYER_EXPORT
void ZEROPLAYER_CALL abortLoad(uint32_t id)
{
    if (!audioInitialized) return;
    LOGE("abortLoad(%d)", id);
    freeAudio(id);
...
ZEROPLAYER_EXPORT
void ZEROPLAYER_CALL finishedLoading(uint32_t id)
{
    if (!audioInitialized) return;
...
ZEROPLAYER_EXPORT
void ZEROPLAYER_CALL pauseAudio(bool _audioPaused)
{
...
ZEROPLAYER_EXPORT
void ZEROPLAYER_CALL initAudio() {
    if (!audioInitialized) {
        ...
        maDevice = (ma_device*) malloc(sizeof(ma_device));
...
ZEROPLAYER_EXPORT
void ZEROPLAYER_CALL destroyAudio() {
    freeAllSources();
    freeAllClips();
    if (audioInitialized) {
...
ZEROPLAYER_EXPORT
uint32_t ZEROPLAYER_CALL playSource(uint32_t clipID, float volume, bool loop)
{
    if (!audioInitialized) return 0;
    flushMemory();
...
ZEROPLAYER_EXPORT
bool ZEROPLAYER_CALL isPlaying(uint32_t sourceID) 
{
...
ZEROPLAYER_EXPORT
bool ZEROPLAYER_CALL stopSource(uint32_t sourceID)
{
    if (!audioInitialized) return false;
