...
namespace Unity.ZeroPlayer
{
    internal class ExecuteGen
    {
        ...
        internal const string DisposeFnName = "Dispose_Gen";
            ...
            bool usesEntity)
        {
            ...
            var getManagerACCCTypeFnDef =
                entityManagerDef.Methods.First(i => i.Name == "GetArchetypeChunkComponentType");
            ...
            var getSystemACCCTypeFnDef =
                componentSystemBaseDef.Methods.First(i => i.Name == "GetArchetypeChunkComponentType");
            ...
            MethodDefinition getNativeArrayFnDef =
            ...
            MethodDefinition unsafePtrFnDef = nativeArrayUnsafeUtilityDef.Methods.First(i => i.Name == "GetUnsafePtr");
            MethodDefinition unsafeReadOnlyPtrFnDef =
            ...
            {
                paramSystem = new ParameterDefinition("system", ParameterAttributes.None, componentSystemRef);
            ...
            var openArchetypeCCTDef =
                m_entityAssembly.MainModule.Types.First(i =>
            ...
            {
                var special =
                    openArchetypeCCTDef.MakeGenericInstanceType(executeParams[i].ParameterType.GetElementType());
                var vd = new VariableDefinition(asm.MainModule.ImportReference(special));
            ...
            else
            {
                ...
                for (int i = 0; i < comps.Count; ++i)
                {
                    ...
                    if (comps[i].access == Access.ReadOnly) methodDef = openReadOnlyDef;
                    else if (comps[i].access == Access.ReadWrite) methodDef = openReadWriteDef;
                    else if (comps[i].access == Access.Exclude) methodDef = openExcludeDef;

                    MethodReference special =
                        TypeRegGen.MakeGenericMethodSpecialization(methodDef, comps[i].typeDefinition);
            ...
            for (int i = 0; i < executeParams.Count; ++i)
            {
                ...
                var specialGetACCCTypeMethodDef = TypeRegGen.MakeGenericMethodSpecialization(
                    isQuery ? getManagerACCCTypeFnDef : getSystemACCCTypeFnDef,
                    executeParams[i].ParameterType.GetElementType());
                bc.Add(Instruction.Create(OpCodes.Callvirt,
                    asm.MainModule.ImportReference(specialGetACCCTypeMethodDef)));
...

            {
                ...
                for (int i = 0; i < executeParams.Count; ++i)
                {
                    ...
                    var specialGetNativeArrayFnRef =
                        TypeRegGen.MakeGenericMethodSpecialization(getNativeArrayFnDef, element);

                    bc.Add(Instruction.Create(OpCodes.Call,
                        asm.MainModule.ImportReference(specialGetNativeArrayFnRef)));

                    MethodReference special = null;
                    if (ParamIsReadOnly(executeParams[i]))
                    {
                        special = TypeRegGen.MakeGenericMethodSpecialization(unsafeReadOnlyPtrFnDef, element);
                    }
                    else
                    {
                        special = TypeRegGen.MakeGenericMethodSpecialization(unsafePtrFnDef, element);
                    }
                ...
                // Execute_TypingGen(job, chunk.Count, (Position*) array0);
            ...
            // Call the existing Deallocate
            if (disposeMethod != null)
            {
                bc.Add(Instruction.Create(OpCodes.Ldarg, paramJob));
                bc.Add(Instruction.Create(OpCodes.Call, disposeMethod));
            }
            ...
            bool usesEntity)
        {
            ...
            var paramJob = new ParameterDefinition("job", ParameterAttributes.None,
                asm.MainModule.ImportReference(jobStruct.Resolve()));
            method.Parameters.Add(paramJob);
        ...
        MethodDefinition GenDisposeMethod(string methodName,
            AssemblyDefinition asm,
            TypeDefinition jobStruct)
        {
            var method = new MethodDefinition(methodName,
            ...
            foreach (var field in jobStruct.Fields)
            {
                ...
                {
                    bc.Add(Instruction.Create(OpCodes.Ldarga, paramJob));
                    JobsGen.GenerateDisposeILForField(field, bc, jobStruct, asm);
                    disposeCalled = true;
                }
            }
        ...
        List<Component> FindComponents(TypeDefinition jobStruct, List<ParameterDefinition> executeParams)
        {
            ...
            for (int i = 0; i < executeParams.Count; ++i)
            {
                ...
                {
                    typeDefinition = executeParams[i].ParameterType.GetElementType().Resolve(),
            ...
            IEnumerable<Instruction> outerList)
        {
            ...
            foreach (var g in giList)
            {
                ...
                {
                    throw new Exception(
                        $"Could not find the expected Schedule/Run function to match '{name}' in type '{type.FullName}'. This is a bug.");
                }
                ...
                try
                {
                    ...
                    // There are two variants of the Outer method, but the Execute and Dispose is unique (and
                    ...
                    var existingDisposeMethod = jobStruct.Methods.FirstOrDefault(m => m.Name == DisposeFnName);
                    ...
                    if (existingOuterMethod == null)
                    {
                        ...
                        // A Dispose_Gen not exist (if there is no Dispose() calls) or
                        // can be shared between Outer_Query_Gen and Outer_System_Gen
                        if (existingDisposeMethod == null)
                        {
                            // Note this can return null if there is nothing to dispose.
                            existingDisposeMethod = GenDisposeMethod(DisposeFnName, asm, jobStruct);
                        }

                        existingOuterMethod = GenOuterMethod(outerName, asm, jobStruct,
                            existingExecuteMethod,
                            existingDisposeMethod,
                            executeParams,
                            isQuery, usesEntity);
