...
namespace Unity.ZeroPlayer
{
    ...
    public class TypeRegGen
    {
        ...
        private bool IsTypeValidForSerialization(TypeDefinition typeDef)
        {
            if (typeDef.CustomAttributes.FirstOrDefault(a => a.AttributeType.FullName == "Unity.Entities.ChunkSerializableAttribute") != null)
                return true;
            ...
            foreach (var field in typeDef.Fields)
            {
                ...
                var fieldRef = field.FieldType;
                if (fieldRef.IsPointer || fieldRef.MetadataType == MetadataType.IntPtr || fieldRef.MetadataType == MetadataType.UIntPtr)
                {
                    return false;
                }

                var fieldDef = field.FieldType.Resolve();
                if (fieldRef.IsValueType && !fieldRef.IsPrimitive && !fieldDef.IsEnum)
                {
                    return IsTypeValidForSerialization(fieldDef);
                }
            }
        ...
        internal TypeGenInfo CreateTypeGenInfo(TypeDefinition type, TypeCategory typeCategory)
        {
            ...
            else if(!isManaged && type != null)
            {
                ...
                alignAndSize = TypeUtils.AlignAndSizeOfType(type, m_ArchBits);
                isSerializable = IsTypeValidForSerialization(type);
            }
        ...
        void RecursePatchCalls(TypeDefinition type, AssemblyDefinition asm, bool usesEntities)
        {
            ...
            {
                RecursePatchCalls(td, asm, usesEntities);
            }
        ...
        internal void PatchCalls()
        {
            foreach (AssemblyDefinition asm in m_AssemblyDefs)
            {
                ...
                {
                    bool usesEntities = mod.AssemblyReferences.FirstOrDefault(x => x.Name == "Unity.Entities") != null;

                    if (usesEntities)
                    {
                        foreach (TypeDefinition t in mod.Types)
                        {
                            RecursePatchCalls(t, asm, usesEntities);
                        }
                    }
                }
            }
        ...
        internal void PushNewTypeInfoArray(ref ILProcessor il, FieldReference fieldRef, TypeGenInfo typeGenInfo)
        {
            ...
            bool isSerializable = typeGenInfo.IsSerializable;
            ...
            EmitLoadConstant(ref il, isSerializable ? 1 : 0);
