...
inline bool HasParentOrIs(TinyType* type, TinyType* targetType)
{
...
inline bool IsAssignableFrom(TinyType* klass, TinyType* oklass)
{
...
inline bool il2cpp_codegen_is_assignable_from(Type_t* left, Type_t* right)
{
    ...
    return IsAssignableFrom(reinterpret_cast<TinyType*>(left), reinterpret_cast<TinyType*>(right));
}
...
{
    const TinyType* tinyType = reinterpret_cast<const TinyType*>(t);
    uint8_t typeHierarchySize = tinyType->typeHierarchySize;
    if (typeHierarchySize == 0)
        return NULL;

    return const_cast<Type_t*>(reinterpret_cast<const Type_t*>(tinyType->GetTypeHierarchy()[typeHierarchySize - 1]));
}

inline MulticastDelegate_t* il2cpp_codegen_create_combined_delegate(Type_t* type, Il2CppArray* delegates, int delegateCount)
{
    Il2CppMulticastDelegate* result = static_cast<Il2CppMulticastDelegate*>(il2cpp_codegen_object_new(sizeof(Il2CppMulticastDelegate), reinterpret_cast<TinyType*>(type)));
...
inline Exception_t* il2cpp_codegen_get_argument_exception(const char* msg)
{
    return NULL;
}
